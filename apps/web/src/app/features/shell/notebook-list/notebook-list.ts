    ...
    <div class="flex-1 overflow-y-auto p-1" cdkDropList (cdkDropListDropped)="onDrop($event)">
      ...
      @for (nb of state.notebooks(); track nb.id) {
        <div
          ...
          [class.bg-accent-100]="nb.id === state.selectedNotebookId()"
          [class.dark:bg-accent-900]="nb.id === state.selectedNotebookId()"
          ...
          [class.ring-accent-300]="state.draggingType() === 'section' && nb.id !== state.selectedNotebookId()"
          [class.dark:ring-accent-600]="state.draggingType() === 'section' && nb.id !== state.selectedNotebookId()"
