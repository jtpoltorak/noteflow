...
@Component({
  ...
  template: `
    <app-modal [open]="true" title="Move note" (closed)="cancelled.emit()">
      <!-- Notebooks list -->
      <div class="space-y-1">
        @if (loading()) {
                  ...
                  @if (loadingSections()) {
                    ...
                    @for (sec of sections(); track sec.id) {
                      @if (sec.id === currentSectionId()) {
                        <div
                          class="flex items-center rounded px-3 py-1.5 text-sm text-gray-400 dark:text-gray-500"
                        >
                          <fa-icon [icon]="faFolder" class="mr-2" size="sm" />
                          <span class="truncate">{{ sec.title }}</span>
                          <span class="ml-auto text-xs italic">(current)</span>
                        </div>
                      } @else {
                        <button
...
export class MoveNoteDialog implements OnInit {
  private notebookSvc = inject(NotebookService);
  private sectionSvc = inject(SectionService);

  currentSectionId = input.required<number>();
  moved = output<number>();
  cancelled = output();
